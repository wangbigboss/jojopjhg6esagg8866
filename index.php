...
{
	$speed=round(100/($_GET['speed']/1000),2); //下載速度：$speed kb/s
}
else
{
	$speed="<font color=\"red\">&nbsp;未探測&nbsp;</font>";
}	
...
function sys_linux()
{
    ...
    $res['memRealUsed'] = $res['memTotal'] - $res['memFree'] - $res['memCached'] - $res['memBuffers']; //真實記憶體使用
	$res['memRealFree'] = $res['memTotal'] - $res['memRealUsed']; //真實空閒
    $res['memRealPercent'] = (floatval($res['memTotal'])!=0)?round($res['memRealUsed']/$res['memTotal']*100,2):0; //真實記憶體使用率

	$res['memCachedPercent'] = (floatval($res['memCached'])!=0)?round($res['memCached']/$res['memTotal']*100,2):0; //Cached記憶體使用率
...
function sys_windows()
{
	...
	$sysinfo[0]['Caption']=iconv('BIG5', 'UTF-8',$sysinfo[0]['Caption']);
	$sysinfo[0]['CSDVersion']=iconv('BIG5', 'UTF-8',$sysinfo[0]['CSDVersion']);
	$res['win_n'] = $sysinfo[0]['Caption']." ".$sysinfo[0]['CSDVersion']." 序號:{$sysinfo[0]['SerialNumber']} 於".date('Y年m月d日H:i:s',strtotime(substr($sysinfo[0]['InstallDate'],0,14)))."安裝";
	//UPTIME
...
$load = $sysInfo['loadAvg'];	//系統負載


//判斷記憶體如果小於1G，就顯示M，否則顯示G單位
if($sysInfo['memTotal']<1024)
{
	...
	$mc = $sysInfo['memCached']." M";	//cache化記憶體
	$mb = $sysInfo['memBuffers']." M";	//緩衝
	$st = $sysInfo['swapTotal']." M";
	$su = $sysInfo['swapUsed']." M";
	$sf = $sysInfo['swapFree']." M";
	$swapPercent = $sysInfo['swapPercent'];
	$memRealUsed = $sysInfo['memRealUsed']." M"; //真實記憶體使用
...
else
{
	...
	$swapPercent = $sysInfo['swapPercent'];
	$memRealUsed = round($sysInfo['memRealUsed']/1024,3)." G"; //真實記憶體使用
...
<table>
  ...
  <tr>
    <td width="13%">CPU型號 [<?php echo $sysInfo['cpu']['num'];?>核]</td>
  ...
  <tr>
    ...
    <td colspan="5">
		...
		空閒 <font color='#333333'><span id="freeSpace"><?php echo $df;?></span></font>&nbsp;G，
...
{
?>		
		  Cache化記憶體為 <span id="CachedMemory">0</span>
		  , 使用率 
          <span id="memCachedPercent">0</span>
		  %	| Buffers緩衝為  <span id="Buffers">0</span>
...
<table>
  ...
  <tr align="center">
    <td align="left">美國 PhotonVPS.com</td>
    <td>0.431秒</td>
    <td>1.024秒</td>
    <td>0.034秒</td>
  ...
  <tr align="center">
    <td align="left">德國 SpaceRich.com</td>
    <td>0.421秒</td>
    <td>1.003秒</td>
    <td>0.038秒</td>
  ...
  <tr align="center">
    <td align="left">美國 IXwebhosting.com</td>
    <td>0.535秒</td>
    <td>1.607秒</td>
    <td>0.058秒</td>
...
<table>
  ...
  <tr>
    ...
    <td width="25%">
      <input class="btn" type="submit" name="act" value="MySQL檢測" />
    </td>
  </tr>
</table>
  <?php
...
<table>
  ...
  <tr>
    ...
    <td width="25%">
    <input class="btn" type="submit" name="act" value="郵件檢測" />
    </td>
  </tr>
  <?php
  if ($_POST['act'] == '郵件檢測') {
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-111725992-1"></script>
<script>
